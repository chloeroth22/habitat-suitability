...
def match(xds, xds_match):
    ...
    xds : xarray.DataArray or xarray.Dataset
        The first xarray dataset or data array to be harmonized. It is the
        reference dataset that will be aligned with the second dataset.

    xds_match : xarray.DataArray or xarray.Dataset
        The second xarray dataset or data array that will be reprojected to
        match the coordinates of `xds`. It is assumed to have compatible x
        and y dimensions.

    Returns
    -------
    xarray.DataArray or xarray.Dataset
        A new xarray object with the same data as `xds`, but with coordinates
        matching the `xds_match` dataset's coordinates. The coordinates for x
        and y reassigned based on `xds_match`.
    """
...
def transform_raster(raster, min_dealbreaker, min_optimal, max_optimal,
                     max_dealbreaker):
    """
    Applies a linear transformation to a raster dataset based on specified
    threshold values.

    The function normalizes the values in the raster by applying a linear
    transformation to values that fall within specified ranges. The
    transformation is defined using two linear functions: one for values below
    the optimal range (min) and one for values above the optimal range (max).
    Values outside of the defined ranges are assigned boundary values (0 or 1).
    ...
    min_dealbreaker : float
        The min value threshold below which all raster values are mapped to 0.

    min_optimal : float
        The min value threshold above which raster values will be linearly
        transformed using the `slope_min` and `intercept_min` values.

    max_optimal : float
        The max value threshold below which raster values will be linearly
        transformed using the `slope_max` and `intercept_max` values.

    max_dealbreaker : float
        The max value threshold above which all raster values are mapped to 0.

    Returns
    -------
    xarray.DataArray
        A new xarray DataArray with the transformed raster values, where values
        outside the specified ranges are clipped to 0 or 1, and values within
        the optimal range are linearly transformed.

    Notes
    -----
    - Values below `min_dealbreaker` are mapped to 0, and values above
      `max_dealbreaker` are also mapped to 0.
    - Values between `min_dealbreaker` and `min_optimal` are linearly scaled
      from 0 to 1.
    - Values between `max_optimal` and `max_dealbreaker` are linearly scaled
      from 1 to 0.
